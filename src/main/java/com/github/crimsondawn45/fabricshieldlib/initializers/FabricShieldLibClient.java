...
public class FabricShieldLibClient implements ClientModInitializer {
     ...
     */
    public static final EntityModelLayer fabric_banner_shield_model_layer = new EntityModelLayer(FabricShieldLib.id("fabric_banner_shield"),"main");
    public static ShieldEntityModel modelFabricShield;
    @SuppressWarnings("deprecation")
    public static final SpriteIdentifier FABRIC_BANNER_SHIELD_BASE = new SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, FabricShieldLib.id("entity/fabric_banner_shield_base"));
    @SuppressWarnings("deprecation")
    public static final SpriteIdentifier FABRIC_BANNER_SHIELD_BASE_NO_PATTERN = new SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, FabricShieldLib.id("entity/fabric_banner_shield_base_nopattern"));
    ...
    public static void renderBanner(ItemStack stack, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay, ShieldEntityModel model, SpriteIdentifier base, SpriteIdentifier base_nopattern){
        ...
        model.getHandle().render(matrices, vertexConsumer, light, overlay, 0xFFFFFFFF);
        if (bl) {
            ...
            model.getPlate().render(matrices, vertexConsumer, light, overlay, 0xFFFFFFFF);
        }
    ...
    public static List<Text> getCooldownTooltip(ItemStack stack, TooltipType type, List<Text> tooltip, int cooldownTicks) {
        ...
        if(type.isAdvanced()) {
            ...
            for(int i = tooltip.size() - 1; i > 0; i--) {
                ...
                if(Identifier.tryParse(strText) != null) {
                    advanced.add(text);
                    tooltip.remove(i);
                    break;
                }
